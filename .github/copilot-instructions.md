...
1. **Environment Management**
   ...
   - Depending on used language, use `python-dotenv`; `usethis` and .Renviron file for environment variable management; never hardcode secrets.
...
3. **Testing**
   - All new code must include unit tests (depending on used programming language; preferably with `pytest` or `testthat`).
   - Maintain >80% code coverage for all modules.

4. **Code Style**
   - Enforce PEP8 with tools like `flake8` or `black`.
